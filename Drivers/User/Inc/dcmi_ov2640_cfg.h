...
//	fanke
// 如果要手动配置,用户需要根据实际输出的图像尺寸去计算具体的 PCLK ,这里以 800*600 分辨率为例：
// 一帧 RGB565（16位色） 图像的数据量为：800*600*2 = 0.96 MB ,
// OV2640在 SVGA模式下,帧率为30左右,则每秒的数据量在 0.96 * 30 = 28.8 MB 左右,
// 因为模块是8位的接口,则PCLK最少要设置为 28.8 MHz,才能满足图像传输的需求,不然会导致花屏  ,
// 加上OV2640的帧率是可以微调的,因此实际的 PCLK 要稍微大些,此处设置为 48M，
// 即  PCLK = 48M / Bit[6:0] = 48 / 0x01 = 48M
   {0xd3, 0x01},   // R_DVP_SP, 设置 PCLK 引脚的时钟
...
// FANKE
// 如果要手动配置,用户需要根据实际输出的图像尺寸去计算具体的 PCLK ,这里以 880*660 分辨率为例：
// 一帧 RGB565（16位色） 图像的数据量为：880*660*2 = 1.16 MB ,
// OV2640在 QVGA模式下,帧率为15左右,则每秒的数据量在 1.16 * 15 = 17 MB 左右,
// 因为模块是8位的接口,则PCLK最少要设置为 17 MHz,才能满足图像传输的需求,不然会导致花屏  ,
// 加上OV2640的帧率是可以微调的,因此实际的 PCLK 要稍微大些,此处设置为 36M，
// 即  PCLK = 72M / Bit[6:0] = 72 / 0x02 = 36M
   {0xd3, 0x02},   // R_DVP_SP, 设置 PCLK 引脚的时钟
